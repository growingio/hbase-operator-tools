...
//    List<HRegionInfo> result = TEST_UTIL.getHBaseAdmin().getTableRegions(TABLE_NAME);
...
//    RegionsMerger merger = new RegionsMerger(TEST_UTIL.getConfiguration());
//    generateTableData();
//    TEST_UTIL.getHBaseAdmin().flush(TABLE_NAME);
//    final int originalCount = TEST_UTIL.countRows(table);
//    merger.mergeRegions(TABLE_NAME.getNameWithNamespaceInclAsString(), 3);
//    List<HRegionInfo> result = TEST_UTIL.getHBaseAdmin().getTableRegions(TABLE_NAME);
...
//    TEST_UTIL.getHBaseAdmin().getTableRegions(TABLE_NAME).forEach(r -> {
